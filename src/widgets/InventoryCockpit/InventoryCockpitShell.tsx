    ...
    <div className="flex shrink-0 items-center gap-1.5">
      <Tooltip content="Закрыть раздел и перейти на главную">
        <button
          ...
          className="h-3 w-3 rounded-full border border-rose-400/40 bg-rose-500/80 shadow-sm shadow-rose-500/20 hover:bg-rose-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-300"
        />
      </Tooltip>
      <Tooltip content={isExpanded ? 'Свернуть рабочую область' : 'Компактный режим уже включён'}>
        <button
          ...
          className="h-3 w-3 rounded-full border border-amber-400/40 bg-amber-500/80 enabled:hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300"
        />
      </Tooltip>
      <Tooltip content={isExpanded ? 'Рабочая область уже развёрнута' : 'Развернуть на весь экран'}>
        <button
          ...
          className="h-3 w-3 rounded-full border border-emerald-400/40 bg-emerald-500/80 enabled:hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300"
        />
      </Tooltip>
    </div>
  );
        ...
        <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-neutral-900/60 px-3 py-2.5 sm:px-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="flex min-w-0 items-center gap-2 border-l border-white/10 pl-3 font-mono">
              ...
              <span className="hidden text-xs font-bold tracking-wider text-white sm:inline">KUMO-CRM</span>
