...
export default function GlobalError({
  ...
  return (
    <html lang="ru">
      <body className="min-h-screen bg-[#0a0a0a] bg-dot-grid text-white" style={globalErrorFonts}>
        <main className="min-h-screen px-3 py-4 font-sans sm:px-6 sm:py-6">
          <section className="mx-auto max-w-2xl overflow-hidden rounded-2xl border border-white/15 bg-neutral-950/90 shadow-[0_20px_80px_-15px_rgba(0,0,0,0.9)] backdrop-blur-2xl">
            ...
            <div className="space-y-4 p-5 sm:p-6">
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <p className="font-mono text-xs text-rose-300">СБОЙ ОБОЛОЧКИ ПРИЛОЖЕНИЯ</p>
                <h1 className="mt-2 text-xl font-semibold text-white">Система временно недоступна</h1>
                <p className="mt-2 text-sm leading-relaxed text-neutral-400">Повторите попытку загрузки. Локальные данные не затрагиваются.</p>
                {error.digest ? <p className="mt-3 font-mono text-[11px] tabular-nums text-neutral-500">TRACE: {error.digest}</p> : null}
              </div>
              <CockpitButton icon={RefreshCw} onClick={retry} title="Перезагрузить приложение">перезагрузить</CockpitButton>
            </div>
